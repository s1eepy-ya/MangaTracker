...
# Read original frames_shop.html
with open('c:/Users/lizae/Downloads/MangaTracker/templates/frames_shop.html', 'r', encoding='utf-8') as f:
    content = f.read()

# Replace text
content = content.replace('frames', 'banners')
content = content.replace('frames_shop', 'banners_shop')
content = content.replace('Frames', 'Banners')
content = content.replace('frame', 'banner')
content = content.replace('Рамки', 'Баннеры')
content = content.replace('Рамка', 'Баннер')
content = content.replace('рамка', 'баннер')
content = content.replace('рамку', 'баннер')
content = content.replace('рамок', 'баннеров')
content = content.replace('рамки', 'баннера')
content = content.replace('AvatarBanner', 'ProfileBanner')

# Now rewrite the preview section
    ...
    <div class="relative bg-cardBg rounded-3xl border border-borderDark overflow-hidden shadow-2xl mb-8">
        ...
        <div class="px-6 md:px-10 pb-6 -mt-16 relative z-10 flex flex-col md:flex-row md:items-end gap-6">
            ...
            <div class="flex-1 text-center md:text-left">
                ...
                {% if current_user.active_banner %}
                <form method="POST" action="{{ url_for('remove_banner') }}" class="inline-block">
...
# Fix the cards to show banner preview instead of avatar-wrapper
...
content = content.replace('Убаннерения', 'Украшения')
content = content.replace('убаннерения', 'украшения')
